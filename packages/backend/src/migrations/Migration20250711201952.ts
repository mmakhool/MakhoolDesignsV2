...
export class Migration20250711201952 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create index "projects_category_index" on "projects" ("category");`
    );
    this.addSql(
      `create index "projects_featured_index" on "projects" ("featured");`
    );
    this.addSql(
      `create index "projects_is_active_index" on "projects" ("is_active");`
    );
    ...
    this.addSql(
      `create index "reviews_featured_index" on "reviews" ("featured");`
    );
    this.addSql(
      `create index "reviews_is_approved_index" on "reviews" ("is_approved");`
    );
  }
}
